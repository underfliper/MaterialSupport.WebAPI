...
applications = pd.read_sql(
    """SELECT a.Id, a.Date, a.Status, s.[Group], s.Gender, s.Birthplace, s.Citizenship, asp.SupportTypeId
...
application = pd.read_sql(
    f"""SELECT a.Id, a.Date, a.Status, s.[Group], s.Gender, s.Birthplace, s.Citizenship
    FROM Applications as a
    JOIN Students as s
    ON a.StudentId=s.Id
...
applications.columns = ['ApplicationId', 'Date', 'Status', 'Group',
                        'Gender', 'Birthplace', 'Citizenship', 'SupportType']
application.columns = ['ApplicationId', 'Date', 'Status', 'Group',
                       'Gender', 'Birthplace', 'Citizenship', 'SupportType']
...
df1 = application.merge(categories.groupby('ApplicationId')['Name']. apply(
    lambda x: x.reset_index(drop=True)). unstack(). reset_index())
df.columns = ['ApplicationId', 'Date', 'Status', 'Group',
              'Gender', 'Birthplace', 'Citizenship', 'SupportType', 'Category1', 'Category2', 'Category3']
df1.columns = ['ApplicationId', 'Date', 'Status', 'Group',
               'Gender', 'Birthplace', 'Citizenship', 'SupportType', 'Category1', 'Category2', 'Category3']
...
def concatenate_features(df_row):
    return f"""{df_row['Gender']};{df_row['Birthplace']};{df_row['Citizenship']};
                {df_row['Category1']};{df_row['Category2']};{df_row['Category3']}"""
